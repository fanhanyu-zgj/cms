...
@section('content')
    @component('components.tabs',['title'=>'栏目管理'])
        @slot('nav')
            <li class="nav-item"><a href="/article/category" class="nav-link ">栏目列表</a> </li>
            ...
            <form action="/article/category/{{$category['id']}}" method="post">
                ...
                <div class="card-body">
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-12 col-sm-8 col-lg-6">
                            <select name="pid"  class="form-control form-control-xs" id="">
                                <option value="0">顶级栏目</option>
                                @foreach($categories as $categoryItem)

                                <option value="{{$categoryItem['id']}}"
                                        {{$categoryItem['_selected']?'selected':''}}
                                        {{$categoryItem['_disabled']?'disabled':''}}
                                >{!! $categoryItem['_name'] !!}</option>
